...
v4: fixes path matching (absolute vs relative), uses openclaw CLI correctly.
"""
...
def translate_via_cli(text, mode='content'):
    ...
    if mode == 'title':
        prompt = (
            "Translate this Chinese chapter title to English. "
            "Return ONLY the English title, no quotes, no explanation, no other text:\n\n"
            + text
        )
    else:
        if len(text) > 8000:
        ...
        prompt = (
            "Translate the following Chinese novel chapter content to English. "
            "Return ONLY the translation, no explanation, no notes, no Chinese text:\n\n"
            + text
        )

    env = dict(os.environ)
    env['OPENCLAW_LOG_LEVEL'] = 'error'
    # Also suppress plugin output via stderr redirect in subprocess
    ...
    if result.returncode != 0:
        raise Exception(f"CLI error (rc={result.returncode}): {result.stderr[:300]}")

    # Parse JSON output - find JSON object in stdout
    output = result.stdout.strip()
    # Try line-by-line first (in case of extra text)
...
def main():
    ...
    # Normalize: convert all paths to absolute for matching
    def norm(p):
        return os.path.normpath(os.path.abspath(p))

    done_set = set(norm(p) for p in done_list)
    failed_set = set(norm(p) for p in failed_list)

    if done_count > 0 or failed_list:
    ...
    for i, ch in enumerate(fake_chapters):
        ...
        ch_path_abs = os.path.abspath(ch['path'])
        if ch_path_abs in done_set:
            print(f"\n  ⏭️ [{i+1}/{len(fake_chapters)}] SKIPPED (already done): {ch['novel']}/{ch['file']}")
            continue
        if ch_path_abs in failed_set:
        ...
        if content_en and not is_chinese(content_en):
            try:
                ...
                done_count += 1
                done_list.append(ch['path'])
                done_set.add(ch_path_abs)
                print(f"   ✅ Translated and saved")
            except Exception as e:
                print(f"   ❌ Save error: {e}")
                failed_list.append(ch['path'])
                failed_set.add(ch_path_abs)
        else:
